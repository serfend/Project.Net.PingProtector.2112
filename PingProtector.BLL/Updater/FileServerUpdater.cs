...
namespace PingProtector.BLL.Updater
{
	public class FileServerUpdater
	{
		...
		public FileServerUpdater(List<string> host)
		{
			this.Host = host;
			CheckUpdate(null);
			checkUpdate = new Timer(new TimerCallback(CheckUpdate), null, 0, 60000);
		}

		public void CheckUpdate(object v)
		{
			var currentUpdateInfo = updateRecord.GetInfo("CurrentUpdate", "2020-1-1");
			DateTime.TryParse(currentUpdateInfo, out var currentUpdate);
			if (currentUpdate == null) currentUpdate = DateTime.Now.AddYears(-1);
			...
			using (var http = new HttpClient())
			{
				try
				{
					...
					if (r.StatusCode == System.Net.HttpStatusCode.OK)
					{
						...
						{
							var id = file.SelectToken("id")?.ToString();
