...
namespace NetworkApi.NetworkManagement
{
	...
	public static partial class NetworkInterfaceInfoExtensions
	{
		public static string ToSummary(this NetworkInterfaceInfo? info)
		{
			...
			var ips = string.Join(',', info.IPv4Address?.Select(i => i.ToString()) ?? new List<string>());
			var gates = string.Join(',', info.IPv4Gateway?.Select(i => i.ToString()) ?? new List<string>());
			var name = info.Name;
		...
		private static Reg? GetIpv4Reg(this NetworkInterfaceInfo? g) => g == null ? null : new Reg(InterfaceIpv4RegistryPath, RegDomain.LocalMachine).In(g.Id ?? "Default");
		...
		{
			if (g?.Description == null) return null;
