using NetworkApi.Network.NetworkChangedDetector;
using System;
...
namespace Project.Core.Protector.BLL.Network.NetworkChangedDetector
{
	...
	{
		private readonly List<string> detectIp;
		...
		public NetworkChangeDetector(List<string> detectIp)
		{
		...
		public void CheckConnection()
		{
			...
			foreach (var ip in detectIp)
			{
				var r = CheckInternet(ip);
				if (r) break;
			}
		...
		private void CheckHost()
		{
			...
			try
			{
				...
				Host.IPAddress = NetworkApi.NetworkInterfaceManagement.NetworkInterface.DetectLocalIPAddressBasedOnRouting(ip)?.ToString();
			}
			catch (Exception ex)
			{
		...
		private void CheckGateway()
		{
			...
			try
			{
				GateWay.IPAddress = NetworkApi.NetworkInterfaceManagement.NetworkInterface.DetectGatewayBasedOnLocalIPAddress(IPAddress.Parse(Host?.IPAddress ?? "0.0.0.0"))?.ToString();
			}
			catch (Exception)
			{
				// ignored
