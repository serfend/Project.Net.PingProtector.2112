...
namespace Common
{
	public class Executor
	{
		...
		/// 可执行字符串数组
		/// </param>
		public Executor(ExecutorItem[] items)
		{
		...
		public Executor(Type returnType, string expression, string name)
		{
			//创建可执行字符串数组
			ExecutorItem[] items = { new ExecutorItem(returnType, expression, name) };
			ConstructEvaluator(items);      //调用解析字符串构造函数进行解析
		}
		...
		public Executor(ExecutorItem item)
		{
			ExecutorItem[] items = { item };//将可执行字符串项转为可执行字符串项数组
			ConstructEvaluator(items);      //调用解析字符串构造函数进行解析
		}
		...
		private void ConstructEvaluator(ExecutorItem[] items)
		{
			//创建C#编译器实例
			var codePro = new CSharpCodeProvider();
			//编译器的传入参数
			...
			foreach (ExecutorItem item in items)               //遍历每一个可执行字符串项
			{
			...
			//得到编译器实例的返回结果
			CompilerResults cr = codePro.CompileAssemblyFromSource(cp, code.ToString());
		...
		{
			Executor eval = new Executor(typeof(int), code, staticMethodName);//生成 Evaluator 类的对像
		...
		{
			Executor eval = new Executor(typeof(string), code, staticMethodName);//生成 Evaluator 类的对像
		...
		{
			Executor eval = new Executor(typeof(bool), code, staticMethodName);//生成 Evaluator 类的对像
		...
		{
			Executor eval = new Executor(typeof(object), code, staticMethodName);//生成 Evaluator 类的对像
	...
	public class ExecutorItem
	{
		...
		/// <param name="expression">执行表达式</param>
		/// <param name="name">执行字符串名称</param>
		public ExecutorItem(Type returnType, string expression, string name)
		{
